...
followers=graph.cypher.execute("MATCH (n)-[r:"+var+"tweets]->(x) WITH n, count(r) as tweet  return n.name,n.id,count(r)")

user_list=[]
date_after=tweet.created_at.date()

#Get min number of tweets
for user in followers:
	aux=int(user[2])
	tweets_till_now=
	print 'min_tweet',min_tweet

# Get User with min number of tweets
for user in max_users:
	if(int(user[2])==min_tweet):
